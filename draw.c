...
void draw_phong(struct matrix *polygons, screen s, zbuffer zb,
	...
	for (point = 0; point < polygons->lastcol - 2; point += 3) {
		...
		if (v > 0) {
		  shade_phong(polygons, point, s, zb,
			      view, light, ambient,
			      reflect);
...
void shade_phong(struct matrix *points, int point, screen s, zbuffer zb,
                 double *view, double light[2][3], color ambient,
                 struct constants *reflect) {
	int bot, mid, top, y;
	...
	//	double n0[3], n1[3];
	double dn0[3], dn1[3]; // normals on two sides while going up; delta normals
	int flip = 0;
	y0 = points->m[1][point];
	...
	*/
	...
	t_to_b = t_y - y; //top ~ bottom distance
	m_to_b = m_y - y; // mid ~ bottom distance
	t_to_m = t_y- m_y; // top ~ mid distance

	dx0 = t_to_b > 0 ? (t_x - b_x) / t_to_b : 0;
	dx1 = m_to_b > 0 ? (m_x - b_x) / m_to_b : 0;
	dz0 = t_to_b > 0 ? (t_z - b_z) / t_to_b : 0;
	dz1 = m_to_b > 0 ? (m_z - b_z) / m_to_b : 0;
	dn0[0] = t_to_b > 0 ? (double)(t_n[0] - b_n[0]) / t_to_b : 0; // top bottom normal x diff
	dn0[1] = t_to_b > 0 ? (double)(t_n[1] - b_n[1]) / t_to_b : 0; // top bottom normal y diff
	dn0[2] = t_to_b > 0 ? (double)(t_n[2] - b_n[2]) / t_to_b : 0; // top bottom normal z diff
	dn1[0] = m_to_b > 0 ? (double)(m_n[0] - b_n[0]) / m_to_b : 0; // middle bottom x diff
	dn1[1] = m_to_b > 0 ? (double)(m_n[1] - b_n[1]) / m_to_b : 0; // middle bottom y diff
	dn1[2] = m_to_b > 0 ? (double)(m_n[2] - b_n[2]) / m_to_b : 0; // middle bottom z diff
	...
	*/
	while (y <= t_y) {
		...
		n1_temp[1] = n1[1];
		n1_temp[2] = n1[2];
		draw_line_with_normal(x0, y, z0, x1, y, z1, s, zb, n0_temp, n1_temp, view, light, ambient, reflect);
		/*
		printf("n0: ");
		print_normal(n0);
		printf("| n1: ");
...
void draw_gouraud(struct matrix *polygons, screen s, zbuffer zb,
	...
	for (point = 0; point < polygons->lastcol - 2; point += 3) {
		...
		if (dot_product(normal, view) > 0) {
			...
			shade_gouraud(polygons, point, s, zb, view, light, ambient, reflect);
