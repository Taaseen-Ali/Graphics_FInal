...
void my_main() {
    ...
    zbuffer zb;
    double step_3d = 100;
    double theta, xval, yval, zval, val;
	...
	while(vn){
	  ...
	  //printf("%d: ",i);
            switch (op[i].opcode) {
	    case SHADING:
	      symbol = op[i].op.shading.p;
	      if (strcmp(symbol->name, "flat") == 0) shading = 0;
	      else if (strcmp(symbol->name, "gouraud") == 0) shading = 1;
	      else if (strcmp(symbol->name, "phong") == 0) shading = 2;
	      break;
				case SPHERE:
					...
					if (shading == 0) draw_polygons(tmp, t, zb, view, light, ambient, reflect);
					else if (shading == 1) draw_gouraud(tmp, t, zb, view, light, ambient, reflect);
					else if (shading == 2) draw_phong(tmp, t, zb, view, light, ambient, reflect);
					tmp->lastcol = 0;
					break;
				case TORUS:
